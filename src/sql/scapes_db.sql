...
-- DATABASE: Scapes
-- MySQL 8.0+
-- Generated from ERD & Data Dictionary v1.0
-- ============================================================
...
CREATE TABLE sessions (
  ...
  COMMENT='Tracking sesi aktif; revoked_at diset saat logout untuk invalidasi token.';
...
CREATE TABLE wallpapers (
  ...
  COMMENT='Entitas utama konten; menyimpan metadata wallpaper beserta status moderasi.';

-- ============================================================
-- 9. wallpaper_tags  (junction table — relasi m:n)
-- ============================================================
...
CREATE TABLE upload_limits (
  id                  INT      NOT NULL AUTO_INCREMENT,
  contributor_id      INT      NOT NULL,
  max_uploads_per_day INT      NOT NULL DEFAULT 5,
  upload_count_today  INT      NOT NULL DEFAULT 0,
  last_reset_at       DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,

  CONSTRAINT pk_upload_limits PRIMARY KEY (id),
  CONSTRAINT uq_upload_limits_contributor UNIQUE (contributor_id),
  CONSTRAINT fk_upload_limits_contributor
    FOREIGN KEY (contributor_id) REFERENCES users (id)
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT chk_upload_limits_max CHECK (max_uploads_per_day >= 0),
  CONSTRAINT chk_upload_limits_count CHECK (upload_count_today >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='Konfigurasi dan counter upload harian per contributor; direset otomatis via last_reset_at.';
...
CREATE TABLE user_api_keys (
  ...
  CONSTRAINT uq_user_api_keys_user_source UNIQUE (user_id, source_id),
  CONSTRAINT fk_user_api_keys_user
    FOREIGN KEY (user_id) REFERENCES users (id)
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_user_api_keys_source
    FOREIGN KEY (source_id) REFERENCES api_sources (id)
    ON DELETE CASCADE ON UPDATE CASCADE,

  INDEX idx_user_api_keys_source_id (source_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='API key pribadi pengguna; disimpan terenkripsi, ditampilkan masked di UI.';
